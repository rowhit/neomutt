...
{
  dot_object_header(fp, ci, "CompressInfo", "#60c0c0");
...
{
  dot_object_header(fp, mdata, "ImapMboxData", "#e0e060");
  dot_type_string(fp, "mbox_name", mdata->mbox_name);
  dot_object_footer(fp);
}
#endif
...
void dot_mailbox_maildir(FILE *fp, struct MaildirMboxData *mdata, struct ListHead *links)
{
  ...
  dot_object_header(fp, mdata, "MaildirMboxData", "#e0e060");
...
void dot_mailbox_mbox(FILE *fp, struct MboxAccountData *mdata, struct ListHead *links)
{
  ...
  dot_object_header(fp, mdata, "MboxAccountData", "#e0e060");
...
{
  dot_object_header(fp, mdata, "NntpMboxData", "#e0e060");
...
{
  dot_object_header(fp, mdata, "NmMboxData", "#e0e060");
...
{
  dot_object_header(fp, mdata, "PopAccountData", "#e0e060");
...
void dot_connections(FILE *fp, struct ConnectionList *cl, struct ListHead *links)
{
  struct Connection *prev = NULL;
  struct Connection *np = NULL;
  TAILQ_FOREACH(np, cl, entries)
  {
    dot_connection(fp, np, links);
    if (prev)
      dot_add_link(links, prev, np, "Connection->next", false);

    // if (np->data)
    // {
    //   dot_account(fp, np->data, links);
    //   dot_add_link(links, np, np->data, "Connection->data", false);
    // }
